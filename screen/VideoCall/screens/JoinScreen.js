...
export default function JoinScreen({setScreen, screens, roomId}) {
  ...
  const joinCall = async id => {
    ...
    localPC.addEventListener('track', e => {
      console.log('masuk sini gk?', e.stream, e.track);
      if (e.track && remoteStream !== e.track) {
        console.log('RemotePC received the track join', e.track);

        remoteMediaStream = remoteMediaStream || new MediaStream();
        remoteMediaStream.addTrack(e.track, remoteMediaStream);
        console.log(
          'remoteMediaStream.getVideoTracks()',
          remoteMediaStream.getVideoTracks(),
        );
